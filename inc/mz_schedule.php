...
function mZ_mindbody_show_schedule( $atts ) {
  ...
  // START caching
	...
	//if(!empty($mz_schedule_data['GetClassesResult']['Classes']['Class'])) {
		...
		global $mbo_dir;
		echo $mbo_dir . 'template/classlist.php';
		include($mbo_dir . 'template/classlist.php');
		return ob_get_clean();

		//echo $mbo_dir . 'template/classlist.php';
		//return get_template_part($mbo_dir . 'template/classlist.php');

	//}
}

		//include( $plugin_url . 'template/classlist.php');

		// $return .= '<table class="mbo-table table-striped">';
		//
