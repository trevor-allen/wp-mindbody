...
class MB_API
{
	...
	*/
	protected function callMindbodyService($serviceName, $methodName, $requestData = [], $returnObject = false, $debugErrors = false)
	{
	    $request = array_merge(['SourceCredentials' => $this->sourceCredentials], $requestData);
	    if (! empty($this->userCredentials)) {
	        $request = array_merge(['UserCredentials' => $this->userCredentials], $request);
	    }
	    $this->client = new SoapClient($this->apiServices[$serviceName], $this->soapOptions);
	    try {
	        $result = $this->client->$methodName(['Request' => $request]);
	        if ($returnObject) {
	            return $result;
	        } else {
  ...
  private function extract_client($n)
  {
	...
	*/
	...
	{
	    $result = $this->callMindbodyService('DataService', 'SelectDataXml', ['SelectSql' => $query], $returnObject, $debugErrors);
